...
> 📖 **상세 가이드**: 프로젝트의 `docs/CLERK_SUPABASE_INTEGRATION.md` 파일에서 통합 설정, RLS 정책, 사용 예제를 확인할 수 있습니다.
...
2. 다음 값들을 복사하여 `.env` 파일에 입력:
   ...
   # Supabase 공식 문서에서는 NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY를 사용하지만,
   # NEXT_PUBLIC_SUPABASE_ANON_KEY도 호환됩니다
...
- `/tasks-example`: Clerk + Supabase 네이티브 통합 예제 (할 일 목록 앱)
- `/instruments`: Supabase 공식 문서 예제 (Quickstart 가이드)
...
- [`docs/CLERK_SUPABASE_INTEGRATION.md`](docs/CLERK_SUPABASE_INTEGRATION.md) - Clerk + Supabase 통합 상세 가이드
- [`docs/SUPABASE_NEXTJS_SETUP.md`](docs/SUPABASE_NEXTJS_SETUP.md) - Supabase + Next.js 설정 가이드 (공식 문서 기반)
- [`docs/CLERK_LOCALIZATION.md`](docs/CLERK_LOCALIZATION.md) - Clerk 한국어 로컬라이제이션 가이드
- [`AGENTS.md`](AGENTS.md) - 프로젝트 아키텍처 및 개발 가이드
