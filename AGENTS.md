...
이 프로젝트는 2025년 권장 방식인 **네이티브 통합**을 사용합니다:
...
**설정 방법**:
1. Clerk Dashboard → Setup → Supabase에서 Clerk Domain 확인
2. Supabase Dashboard → Authentication → Providers → Third-Party Auth에서 Clerk 추가
3. Issuer URL: `https://your-app.clerk.accounts.dev`
4. JWKS URI: `https://your-app.clerk.accounts.dev/.well-known/jwks.json`

자세한 내용은 `docs/CLERK_SUPABASE_INTEGRATION.md` 참고.
...
supabase/migrations/20250101000000_create_tasks_table_with_rls.sql
```
...
- `auth.jwt()->>'sub'`를 사용하여 Clerk user ID 추출
- `TO authenticated` 역할에만 정책 적용 (Clerk 토큰이 있는 사용자)
- 예제: `supabase/migrations/20250101000000_create_tasks_table_with_rls.sql` 참고
